...
public class AlumnoBean implements AlumnoInterface{
    ...
    public AlumnoInterface getAlumnoPorNUMEXPDTE(String NUMEXPDTE) {
        ...
        try {
            ...
            conexionA.close();
                ...
                try {
                    // Lanzar la excepción para que el método que lo llama pueda manejarla
                    throw e;
    ...
    public java.util.Collection getAlumnoPorNombre(String nombre) {
        ...
        try {
            ...
            // Modifica la consulta para buscar solo el primer nombre
            resultado = sentencia.executeQuery("SELECT * FROM ALUMNO WHERE SUBSTRING_INDEX(NOMBRE, ' ', 1) = '" + nombre + "'");
    ...
    /*public void delete() {
        ...
        if (alumnos == null || alumnos.isEmpty()) {
            System.out.println("No hay alumnos para borrar.");
            return;
        }

        try {
            conexionA = getConexionAlumno();
            java.sql.Statement sentencia = conexionA.createStatement();

            for (AlumnoInterface alumno : alumnos) {
                sentencia.execute("DELETE FROM ALUMNO WHERE NUMEXPDTE='" + alumno.getNumexpdte() + "'");
            }

            sentencia.close();
            conexionA.close();
        } catch (SQLException e) {
            System.out.println("Error al borrar alumnos: " + e.getMessage());
        }
    }
    ...
    public AlumnoInterface getNuevoAlumno(String NUMEXPDTE, String nombre, String dni, String ciclo) {
        ...
        try {
            sentencia = conexionA.createStatement();
            sentencia.execute("INSERT INTO alumno(numexpdte,nombre,dni,ciclo) VALUES ('" + NUMEXPDTE + "','" + nombre + "','" + dni + "','" + ciclo + "')");
